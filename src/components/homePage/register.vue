<template>
  <div id="login-main">
    ...
    <div id="login-content">
       <input class="myInput" placeholder="邮箱/手机号码" style="width: 320px" v-model="user.userPhone" />
      <input class="myInput" placeholder="昵称" style="width: 320px;margin-top:10px;" v-model="user.userName" />
      <input
        class="myInput"
        placeholder="密码"
...
export default {
  ...
  methods: {
    ...
    registerBtn() {
      let then = this;
      if(this.confirmPwd != this.user.userPwd) return this.errMsg = "两次密码不匹配";
      if (this.user.userName == null) return (this.errMsg = "用户名不能为空");
      //console.log(this.user);
      let fd = new FormData();
      fd.append("userInfo",JSON.stringify(this.user));
      fd.append("userIcon",this.videoCoverImg);
      this.$axios({
        method: "post",
        data: fd,
        url: "/api/controller/register.php"
      }).then(response => {
